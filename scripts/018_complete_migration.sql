-- Script de migración completo que maneja todos los casos
-- Este script es más robusto y maneja columnas faltantes

-- 1. Asegurar que las tablas tengan todas las columnas necesarias
...
BEGIN
    ...
    -- Agregar columnas a bands si no existen
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bands' AND column_name = 'cover_image_url') THEN
        ALTER TABLE public.bands ADD COLUMN cover_image_url text;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bands' AND column_name = 'website_url') THEN
        ALTER TABLE public.bands ADD COLUMN website_url text;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bands' AND column_name = 'spotify_url') THEN
        ALTER TABLE public.bands ADD COLUMN spotify_url text;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bands' AND column_name = 'youtube_url') THEN
        ALTER TABLE public.bands ADD COLUMN youtube_url text;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bands' AND column_name = 'instagram_url') THEN
        ALTER TABLE public.bands ADD COLUMN instagram_url text;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bands' AND column_name = 'location') THEN
        ALTER TABLE public.bands ADD COLUMN location text;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bands' AND column_name = 'genres') THEN
        ALTER TABLE public.bands ADD COLUMN genres text[];
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bands' AND column_name = 'founded_date') THEN
        ALTER TABLE public.bands ADD COLUMN founded_date date;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bands' AND column_name = 'is_active') THEN
        ALTER TABLE public.bands ADD COLUMN is_active boolean DEFAULT true;
    END IF;
END $$;

-- 2. Migrar usuarios existentes (personas que no son bandas)
INSERT INTO public.users (
  id, username, display_name, bio, avatar_url, website_url, location, genres, created_at, updated_at
)
SELECT 
  id, username, display_name, bio, avatar_url, website_url, location, genres, created_at, updated_at
...
-- 4. Crear relaciones usuario-banda para usuarios que tenían bandas
...
-- 5. Crear preferencias por defecto para usuarios existentes
...
-- 6. Crear vistas útiles para consultas comunes
...
UNION ALL
SELECT 
  'User preferences created:' AS info,
  COUNT(*) AS count
FROM public.user_preferences;

-- 8. Mostrar algunos ejemplos de datos migrados
