...
export default async function BandasPage({ searchParams }: BandasPageProps) {
  ...
  const bands = await getBands(20, search)

  const filteredBands = genre
    ? bands.filter((band) => band.genres?.some((g) => g.toLowerCase().includes(genre.toLowerCase())))
    : bands
          ...
          <>
            <div className="flex items-center justify-between mb-6">
              <p className="text-muted-foreground">
                {search || genre
                  ? `${filteredBands.length} resultados encontrados`
                  : `${filteredBands.length} artistas encontrados`}
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredBands.map((band) => (
                <ProfileCard key={band.id} band={band} />
              ))}
            </div>
          </>
        )}
